...
class ReservationChangeMailer < ApplicationMailer
  # 予約リクエストを送付した人
  def send_request_for_visit(user, reservation)
    ...
    mail to: @user.email, subject: '【Gardenia事務局】予約内容が変更されました。再度予約リクエストを送信しています', from: '"Gardenia事務局" <gardenia@gmail.com>'
  end
  ...
  def recieve_request_for_visit(user, reservation)
    ...
    mail to: @reservation.post_garden.user.email, subject: '【Gardenia事務局】予約内容が変更されました。再度予約リクエストが届いています', from: '"Gardenia事務局" <gardenia@gmail.com>'
  end
end
